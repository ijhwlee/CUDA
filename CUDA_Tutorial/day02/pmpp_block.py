...
# define conversion function in emulating block of threads
def block_kernel(f, blocks, threads, *args):
    for i in range(blocks):
        for j in range(threads):
            f(i, j, threads, *args)

def rgb2grey_block(blockidx, threadidx, blockdim, x, out, n):
    i = blockidx * blockdim + threadidx
    if i < n:
        out[i] = 0.2989*x[i] + 0.5860*x[i+n] + 0.1140*x[i + 2*n]

def rgb2grey_pyblock(x):
    c,h,w = x.shape
    n = h*w
    x = x.flatten()
    threads = 512
    ...
    block_kernel(rgb2grey_block, blocks, threads, x, res, n)
    return res.view(h,w)
...
print(f'Elapsed time in Emulating Blocked Kernel with size {w}X{h}: {end_time - start_time:.3f} seconds')
